...
```mermaid
flowchart LR
    A[Cycle n: encounter local resources] --> B[Update retained resource information]
    B --> C[Return with current body and world intact]
    C --> D[Cycle n+1: choose using retained experience]
    D --> E[Measure viability on later cycles]
    B --> F[Matched erase or wrong-history intervention]
    F --> C
```
