...
The [R1 result](../evidence/ol4_t0a_r1_preflight_result.json) is a `PASS` for nine readiness gates, including one deterministic CUDA Adam step on unrelated seed `9101` and a verified checkpoint hash. This establishes that the repaired path can execute under the selected deterministic backend. Its two synthetic lives do not measure learned capability. The registered eight training units and their development outcomes remain unopened.
