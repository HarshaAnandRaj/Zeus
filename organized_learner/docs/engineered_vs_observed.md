...
At the OL3 checkpoint, outer training, learned specialization, a matched partition advantage, broader relational induction, and natural language were unobserved. OL4's later development result is recorded below.
...
The T0 reward estimator passed a 64-branch complete-life gradient check. Its direct-only entropy bonus omitted how earlier actions alter later entropy through public rule updates; a legal diagnostic program produced a missing gradient of norm 0.0184. This was a **T0 preflight FAIL**, with no outer optimization run opened at that checkpoint. The [T0a amendment](v4_t0a_amendment.md) repaired the full trajectory estimator, made world reset explicit, narrowed optimizer inputs, and required boundary, route, and shuffled-source checks before optimization.

T0a subsequently passed all eight [source-hashed preflight gates](../evidence/ol4_t0a_preflight_result.json): 78 mechanics tests, 87/87 raw coordinate support with rank 64 and 23 explained gauges, exact production-loss gradient comparison over 64 complete action branches, explicit world/reset and route checks, and a nontrivial shuffle on the frozen development identities. A separate [unregistered deterministic CUDA smoke](../evidence/ol4_t0a_execution_smoke_fail.json) then failed before its first optimizer step because the installed CUDA cumulative-sum kernel lacks deterministic support. [R1](v4_t0a_r1_execution_repair.md) used equivalent fixed three-query sums and passed a new execution gate.

The [R1 result](../evidence/ol4_t0a_r1_preflight_result.json) is a `PASS` for nine readiness gates, including one deterministic CUDA Adam step on unrelated seed `9101` and a verified checkpoint hash. That smoke's two synthetic lives did not measure learned capability. The later registered [development result](../evidence/ol4_t0a_development/final_result.json) is a separate functional `PASS`: four full programs trained from chance-level early batches to approximately 0.999 late-batch mean reward and achieved 4092–4093/4096 primary sampled successes on fixed development lives. Every acute owner-write lesion had a 99% paired-bootstrap lower effect above 0.4739. Shuffled public teaching and independently trained no-write controls stayed below 0.2766 and 0.2724 respectively on their 99% Wilson upper bounds. This supports within-life acquisition and causal reliance on the four installed owner routes in the engineered task family. It does not establish a unique meaning for 87 raw coordinates because 23 are gauge directions, nor held-out transfer or a compartment advantage.

### OL4 in plain language

We installed the places where the learner may remember a marked location, keep a slow cue, associate a word with a lamp state, and infer an actuator rule. We also installed how those four messages are combined into four candidate two-step plans. Across many simulated lifetimes, outer optimization set the inherited numbers that make those places update and communicate usefully. In each fresh development life, the trained system used public examples, waited through distracting events, acted, and adapted to a later public correction. Removing any one source's writes made the paired behavior materially worse. Training the same allocation with all lifetime writes disabled stayed at chance; shuffling its public lessons also removed the full model's success. The learner did not create the compartments, invent the task grammar, or discover a new planning operator.
...
- Held-out transfer of the four trained inherited programs to prospectively untouched complete lives, with run-level inference.
