...
**Current status:** OL2 remains frozen after its sampled-action failure and inactive-route audit. OL3 passed its registered sampled integration gate as a hand-set solver. OL4-T0 failed pre-optimization prosecution. T0a passed its eight registered readiness gates, then failed a separate deterministic CUDA execution smoke before any registered optimization. T0a-R1 repaired that execution defect and passed all nine new source-hashed readiness gates. The fixed eight-unit development training assay is next. No outer-trained inherited program or architecture comparison exists yet.
...
| [R1 preflight result](evidence/ol4_t0a_r1_preflight_result.json), [review](docs/review_08_ol4_t0a_r1_preflight.md), and [development protocol](evidence/ol4_t0a_development_protocol.md) | Nine readiness gates pass; development identities and eight-unit decision frozen, functional outcome unopened |
