"""SN1R: fresh token windows; preserves failed SN1 and unchanged core source."""
import json
...
OUT=ROOT/'runs/sn1r_20260907'


def prepare(model,prompt,seed,skip_pad):
...
def main():
    ...
    paths=[config_path,checkpoint,ROOT/'core/model.py',Path(__file__),ROOT/'tools/sn1_direct_observation_20260907.py',
        ROOT/'docs/sn1_direct_observation_protocol_20260907.md',ROOT/'docs/sn1r_direct_observation_protocol_20260907.md',ROOT/'runs/sn1_20260907/invalid.json']
    ...
    try:
        ...
        for prompt in P.PROMPTS:
            group=[]
            for seed in (7,8):
                snapshot=prepare(model,prompt,seed,config.get('skip_pad_window',False))
        ...
        report=dict(kind='SN1R_DIRECT_OBSERVATION',diagnostic_only=True,prior_attempt='SN1_INVALID_TOKEN_BUFFER_MISMATCH',manifest=manifest,
            ...
            variance=dict(full=vf,zero_both=vz,residual=vr,covariance=cov,residual_to_full=vr/vf,zero_to_full=vz/vf,
                covariance_term_to_full=2*cov/vf,identity_residual=identity),observation_sha=P.C.sha(OUT/'observations.pt'))
