...
def main():
    ...
    below_resolution=0;branch_count=0;lag_ties=0
    for row in records[3]['cases']:
        ...
        for name,v in row['branches'].items():
            ...
            if rms<1e-10:below_resolution+=1
                ...
                close(v['ratios'],ratios)
                assert v['best_lag']==int(O.LAGS[1+np.argmin(v['ratios'][1:])])
                native_lag=int(O.LAGS[1+np.argmin(ratios[1:])])
                if v['best_lag']!=native_lag:
                    # Full curves already agree. A label may differ at tied
                    # minima; require the recorded minimum also be a native
                    # minimum within the existing absolute comparison tolerance.
                    index=int(np.flatnonzero(O.LAGS==v['best_lag'])[0])
                    assert abs(ratios[index]-min(ratios[1:]))<=1e-10
                    lag_ties+=1
    ...
    result=dict(kind='OBS2R_COMPLETION_AUDIT',passed=all(checks.values()),checks=checks,
        native_replay_branches=branch_count,below_resolution_return_profiles=below_resolution,numerically_tied_lag_labels=lag_ties,
