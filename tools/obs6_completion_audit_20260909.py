...
def main():
    ...
    for trial in C.R.TRIALS:
        for kind in ('final','initial'):
            with np.load(C.OUT/f'{trial}_{kind}.npz') as arrays:d={k:arrays[k] for k in arrays.files}
            with np.load(C.B.OUT/f'{trial}_{kind}.npz') as arrays:old={k:arrays[k] for k in arrays.files}
            rows=C.R.N.read(C.OUT/f'{trial}_{kind}.json')['cases'];p=cache['checkpoints'][trial][kind]
            ...
            # Native forward-mode AD differentiates the same full 496-step map
            # only along the three required directions; no dense reverse Jacobian.
            mapped=np.array([torch.func.jvp(endmap,(m,),(torch.tensor(v),))[1].detach().numpy() for v in d['directions'][ci]])
    ...
    audit=dict(kind='OBS6_AUDIT',passed=all(checks.values()),checks=checks,combinations=count,branches=9472,
               ...
               derivative_method='native forward-mode AD through complete 496-step map',
