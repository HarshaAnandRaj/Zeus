"""Separately arranged integer gates and paired contrasts for the frozen LMB5."""
import math
import numpy as np
...
def decide(endpoints,preps,config):
    ...
    contrasts=[];warm=[]
    for e in (.12,.20):
        difference=[[int(maps['balanced'][t,e,i,'inherited']['survived'])-int(maps['original'][t,e,i,'inherited']['survived']) for i in range(n)] for t in range(nt)]
        contrasts.append(dict(energy=e,**effect(difference,config['attribution_min'])))
    ...
    return dict(verdict='PASS' if selected is not None else 'FAIL',selected_qualified_arm=selected,arms=arms,
        ...
        warm_contrasts=warm,pillar_promotion=False,scope='Fresh scarce-birth motor/readout qualification only; no native memory-benefit result')
