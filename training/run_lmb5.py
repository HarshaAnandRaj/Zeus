"""Fresh equal-budget physical birth exposure; no repair of the exposed LBT2."""
import argparse,gzip,json,subprocess,sys
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];sys.path.insert(0,str(ROOT))
import numpy as np
import torch
from training import scarce_birth_training as S,lmb5_contract as K,diagnose_lbt2_failures as D
R=S.R;M=R.M;C=S.C
OUT=ROOT/'runs/lmb5_20260913';REPORT=ROOT/'zeus_sandbox/universe/reports/lmb5_20260913.json'
SOURCES=tuple(dict.fromkeys((*R.SOURCES,*D.SOURCES,'training/scarce_birth_training.py','training/test_scarce_birth_training.py',
    'training/lmb5_contract.py','training/run_lmb5.py','training/lmb5_judgment.py','training/audit_lmb5.py','training/test_lmb5.py','docs/lmb5_protocol_20260913.md')))


def prerequisites():
    manifest,audit=D.prerequisites();diagnosis=M.L.P.L3.read(D.REPORT)
    assert manifest['source']['arm']=='grounded'
    assert diagnosis['status']=='COMPLETE' and diagnosis['verdict']=='FAIL' and diagnosis['pillar_promotion'] is False
    assert diagnosis['transfer_audit_sha']==M.L.P.L3.sha(D.AUDIT) and diagnosis['source']==manifest['source']
    assert diagnosis['source_preparations']==512 and diagnosis['bodies']==2048 and diagnosis['fatal_bodies']==639
    assert diagnosis['steps_replayed']==audit['decisions'] and diagnosis['source_steps_replayed']==12288
    assert len(diagnosis['body_summaries'])==2048 and len(diagnosis['fatal_traces'])==639
    assert diagnosis['sources']=={p:M.L.P.L3.sha(ROOT/p) for p in D.SOURCES}
    for name in D.SOURCES:
        blob=subprocess.check_output(['git','show',diagnosis['commit']+':'+name],cwd=ROOT)
        assert blob.replace(b'\r\n',b'\n')==(ROOT/name).read_bytes().replace(b'\r\n',b'\n')
    return dict(source=manifest['source'],transfer_audit_sha=M.L.P.L3.sha(D.AUDIT),diagnosis_sha=M.L.P.L3.sha(D.REPORT))


def prepare():
    source=prerequisites();assert not OUT.exists(),'existing campaign preserved'
...
def verify():
    source=prerequisites();m=M.L.P.L3.read(OUT/'manifest.json')
...
def calibration_decide(data,config=K.CONFIG):
    ...
    for arm in config['arms']:
        rows=data[arm];assert len(rows)==len(config['energies'])*config['calibration_ecologies']*2
        expected=[(slot,i,inherited) for slot in range(4) for i in range(config['calibration_ecologies']) for inherited in (True,False)]
        assert [(d['profile_slot'],d['preparation']['index'],d['inherited']) for d in rows]==expected
        for slot,nominal in enumerate(config['energies']):
            for inherited in (True,False):
                rs=[d for d in rows if d['profile_slot']==slot and d['inherited']==inherited]
                actual=nominal if arm=='balanced' else .85
                assert all(d['preparation']['energy']==actual for d in rs)
                survivors=sum(not d['records'][-1]['terminated'] for d in rs)
                expected_survivors=len(rs)//2 if actual==.12 and not inherited else len(rs)
                key=f'{arm}_{slot}_{inherited}';gates[key]=survivors==expected_survivors
                cells.append(dict(arm=arm,slot=slot,nominal_energy=nominal,actual_energy=actual,inherited=inherited,
                    n=len(rs),survivors=survivors,expected_survivors=expected_survivors))
    return dict(verdict='PASS' if all(gates.values()) else 'VOID',gates=gates,cells=cells,
        scope='Prospective public-teacher reachability only; no neural result')


def calibrate():
    verify();data={}
    for arm in K.CONFIG['arms']:
        data[arm]=S.demonstrations(arm,K.CONFIG,base=K.CONFIG['calibration_base'],n=K.CONFIG['calibration_ecologies'])
        C.save(OUT/f'{arm}_calibration_public.json',data[arm])
    result=calibration_decide(data);C.save(OUT/'calibration.json',result);print('scarce-birth teacher calibration',result['verdict'],flush=True)
...
def calibration_qualified():
    ...
    names={'manifest.json','calibration.json','balanced_calibration_public.json','original_calibration_public.json'}
...
def train():
    verify();calibration_qualified()
    for arm in K.CONFIG['arms']:
        data=[]
        for twin in K.CONFIG['twins']:
            actual=S.demonstrations(arm,K.CONFIG);C.save(OUT/f'{arm}_training_{twin}.json',actual)
            data.append(actual)
        assert M.L.P.L3.tree_hash(data[0])==M.L.P.L3.tree_hash(data[1])
        assert M.L.P.L3.sha(OUT/f'{arm}_training_a.json')==M.L.P.L3.sha(OUT/f'{arm}_training_b.json')
        for trial in range(K.CONFIG['trials']):
            for twin,actual in zip(K.CONFIG['twins'],data):S.fit(trial,arm,twin,actual,K.CONFIG,OUT/f'{trial}_{arm}_{twin}')
        del data


def trained(trial,variant):
    body,heads=S.initial(trial)
    if variant!='warm':
...
def evaluate():
    verify();calibration_qualified()
    preps=preparations(K.CONFIG['evaluation_base'],K.CONFIG['evaluation_ecologies']);C.save(OUT/'endpoint_public.json',preps)
    regression=preparations(K.CONFIG['regression_base'],K.CONFIG['regression_ecologies']);C.save(OUT/'regression_public.json',regression)
        ...
        with M.L.trace_writer(OUT/f'{variant}_trace.jsonl.gz') as stream:
            for trial in range(K.CONFIG['trials']):
                ...
                for prep,mode in cases(preps,variant):
                    ...
                    bodies.append(operate(body,trial,variant,prep,mode,emit=lambda row:M.L.log(stream,dict(**tags,**row))))
                if variant!='warm':queries.append(readout(body,trial,regression))
                assert before==M.L.P.L3.tree_hash(body.state_dict()) and head==M.L.P.L3.tree_hash(heads.state_dict())
                hashes.append(dict(trial=trial,model=before,heads=head));print('scarce-birth endpoint',variant,trial,flush=True)
...
def decide(endpoints,preps,config=K.CONFIG):
    ...
    for e in (.12,.20):
        delta=np.array([[int(lookups['balanced'][t,e,i,'inherited']['survived'])-int(lookups['original'][t,e,i,'inherited']['survived']) for i in range(n)] for t in range(nt)])
        bounds=np.quantile(delta[parents[:,:,None],ids[:,None,:]].mean((1,2)),[.025,.975]).tolist();mean=float(delta.mean())
        contrasts.append(dict(energy=e,mean=mean,bounds=bounds,passed=bool(mean>=config['attribution_min'] and bounds[0]>0)))
    ...
    return dict(verdict='PASS' if selected else 'FAIL',selected_qualified_arm=selected,arms=judgments,
        exposure_attribution='PASS' if all(r['passed'] for r in contrasts) else 'FAIL',contrasts=contrasts,
        warm_contrasts=warm_contrasts,pillar_promotion=False,scope='Fresh scarce-birth motor/readout qualification only; no native memory-benefit result')


def finalize():
    manifest=verify();endpoints={variant:M.L.P.L3.read(OUT/f'{variant}_endpoint.json') for variant in (*K.CONFIG['arms'],'warm')}
    result=decide(endpoints,M.L.P.L3.read(OUT/'regression_public.json'));C.save(OUT/'verdict.json',result)
    C.save(REPORT,dict(**result,manifest=manifest,independent_audit_required=True));print('LMB5 raw',result['verdict'],result['exposure_attribution'],flush=True)
