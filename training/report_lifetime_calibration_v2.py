"""Publish quality-world calibration with explicit scripted-controller limits."""
import json
from pathlib import Path

ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'runs/lifetime_calibration_v2_20260909'


def main():
    read=lambda name:json.loads((OUT/name).read_text(encoding='utf-8'))
    result=read('results.json');audit=read('audit.json');complete=read('completion.json');assert audit['passed']
    compact={k:v for k,v in result.items() if k!='episodes'}
    compact['episodes']=[{k:v for k,v in r.items() if k not in ('initial','final')} for r in result['episodes']]
    for name,data in [('results',compact),('audit',audit),('completion',complete)]:
        with (ROOT/f'zeus_sandbox/universe/reports/lifetime_calibration_v2_{name}_20260909.json').open('x',encoding='utf-8') as f:
            json.dump(data,f,separators=(',',':'),allow_nan=False)
    table='| Controller | Stable survivors | Changing survivors | Changing inspections | Changing contaminated harvests |\n| --- | --- | --- | --- | --- |\n'
    for name,c in result['aggregates']['changing'].items():
        s=result['aggregates']['stable'][name]
        table+=f"| {name} | {s['survivors']}/32 | {c['survivors']}/32 | {c['inspections']} | {c['unsafe_harvests']} |\n"
    note=f'''# Lifetime world v2: information and revision calibration PASS
...
current-map erasure. The audit covers all {audit['transitions']:,} transitions across
...
'''
    (ROOT/'docs/lifetime_calibration_v2_review_20260909.md').write_text(note,encoding='utf-8')
    print('Wrote v2 review and compact evidence')
