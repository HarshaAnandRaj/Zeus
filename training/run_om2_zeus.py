...
SOURCES=('core/memory_quality_agent.py','core/quality_agent.py','core/persistent_agent.py',
         ...
         'training/run_om2_zeus.py','training/audit_om2_zeus.py','training/test_om2_zeus.py',
         'docs/om2_protocol_20260912.md')
CONFIG=dict(batch=8,rollout=128,updates=64,lr=.0003,weight_decay=.01,clip=1.,bptt=64,
            ...
            initialization_base=203212000,train_action_base=203312000,
            train_memory_base=203412000,shuffle_base=203512000,
            evaluation_action_base=203612000,evaluation_memory_base=203712000,
            bootstrap_seed=203812000,bootstrap_draws=10000,trials=4,
            arms=ARMS,twins=('a','b'))
...
def stop_disabled_operation_gradients(model,arm):
    for module,disabled in ((model.writer,arm=='no_writer_credit'),
                            (model.reader,arm=='no_reader_credit')):
        if disabled:
            for parameter in module.parameters():parameter.grad=None
...
def train(trial,arm,path):
    ...
    arng=torch.Generator().manual_seed(CONFIG['train_action_base']+trial)
    mrng=torch.Generator().manual_seed(CONFIG['train_memory_base']+trial)
    srng=torch.Generator().manual_seed(CONFIG['shuffle_base']+trial)
    counter=0
    def fresh():
        nonlocal counter
    ...
    state=model.initial(CONFIG['batch']);ticks=np.zeros(CONFIG['batch'],int);episodes=[];updates=[]
    with trace_writer(path/'trace.jsonl.gz') as trace:
        for update in range(CONFIG['updates']):
            ...
            rewards=[];terminated=[];truncated=[];targets=[];bootstrap=[]
            for t in range(CONFIG['rollout']):
                if t%CONFIG['bptt']==0:state['h']=state['h'].detach()
                ...
                for lane,w in enumerate(worlds):
                    ...
                    cut.append(bool(ticks[lane]==CONFIG['horizon'] and not effect.terminated))
                ...
                peek=torch.zeros(CONFIG['batch'])
                if t==CONFIG['rollout']-1 or any(cut):
                    peek_arng=torch.Generator();peek_arng.set_state(arng.get_state())
                    peek_mrng=torch.Generator();peek_mrng.set_state(mrng.get_state())
                    with torch.no_grad():
                        _,_,peek_result=model.act(torch.tensor(after,dtype=torch.float32),newstate,
                                                  peek_arng,peek_mrng)
                        peek=peek_result['value']
                bootstrap.append(peek)
            ...
            next_values=torch.empty_like(stack['value'])
            next_values[:-1]=stack['value'][1:].detach();next_values[-1]=bootstrap[-1]
            trunc=torch.stack(truncated)
            for t in range(CONFIG['rollout']):
                next_values[t]=torch.where(trunc[t],bootstrap[t],next_values[t])
            advantage,targets_v=generalized_advantages(torch.stack(rewards),stack['value'],next_values,
                torch.stack(terminated),torch.stack(truncated),gamma=CONFIG['gamma'],gae_lambda=CONFIG['gae_lambda'])
            ...
            loss=(actor+CONFIG['value_weight']*value+CONFIG['prediction_weight']*prediction
                  -CONFIG['action_entropy']*stack['actor_entropy'].mean()
                  -CONFIG['memory_entropy']*stack['memory_entropy'].mean())
            if arm!='no_writer_credit':loss=loss+writer
            if arm!='no_reader_credit':loss=loss+reader
            opt.zero_grad(set_to_none=True);loss.backward()
            stop_disabled_operation_gradients(model,arm)
...
def evaluate(trial,arm,weights,changing):
    ...
    mrng=torch.Generator().manual_seed(CONFIG['evaluation_memory_base']+trial*100+int(changing))
    trace=[]
...
def decide(results):
    ...
    effects={}
    for arm in controls:
        x=np.stack([np.concatenate([alive256(lookup[t,'full',c]).astype(float)-alive256(lookup[t,arm,c]).astype(float) for c in (False,True)]) for t in range(trials)])
...
def main():
    ...
    manifest=dict(config=json.loads(json.dumps(CONFIG)),sources={p:sha(ROOT/p) for p in SOURCES},
                  parents={str(t):parent(t)[1] for t in range(CONFIG['trials'])},
