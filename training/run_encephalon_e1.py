"""Commit-bound E1 fitting, deterministic repeats and locked endpoint execution."""
import argparse
...
from training.encephalon_e1 import Fit, deterministic, evaluate, read_checkpoint, tree_hash, write_checkpoint
...
def job_directory(route, lineage, twin):
    return K.OUT / f"{route}_{lineage:02d}_{twin}"
...
def fit_job(route, lineage, twin):
    deterministic()
    manifest = verify_manifest()
    directory = job_directory(route, lineage, twin)
    ...
    if checkpoints:
        ...
        assert packet["config"] == K.CONFIG and packet["route"] == route and packet["lineage"] == lineage and not packet["development"]
        assert packet["update"] <= K.CONFIG["updates"]
        fit = Fit.restore(packet)
    else:
        fit = Fit(route, lineage)
    ...
    payload = dict(route=route, lineage=lineage, twin=twin, update=fit.update,
...
def endpoint_job(route, lineage, twin):
    deterministic()
    manifest = verify_manifest()
    directory = job_directory(route, lineage, twin)
    ...
    for control in K.CONFIG["controls"]:
        for profile in K.CONFIG["profiles"]:
            ...
            packets.append(evaluate(model, route, lineage, profile, control))
    verify_manifest()
    save_gzip(target, packets)
    receipt = dict(route=route, lineage=lineage, twin=twin, sha256=sha(target),
...
def run(resume=False):
    ...
    jobs = [(route, lineage, twin) for lineage in range(K.CONFIG["lineages"])
            for route in K.CONFIG["routes"] for twin in K.CONFIG["twins"]]
    with campaign_lock():
        with ProcessPoolExecutor(max_workers=K.CONFIG["workers"]) as pool:
            ...
            for count, future in enumerate(as_completed(futures), 1):
                ...
                print(f"FITTED {count}/{len(jobs)} {result['route']} {result['lineage']} {result['twin']} updates={result['update']}", flush=True)
            ...
            for route in K.CONFIG["routes"]:
                for lineage in range(K.CONFIG["lineages"]):
                    identities = [read(job_directory(route, lineage, twin) / "fit.json")["final_sha256"] for twin in K.CONFIG["twins"]]
                    assert len(set(identities)) == 1, "training twins diverged"
            futures = [pool.submit(endpoint_job, *job) for job in jobs]
            for count, future in enumerate(as_completed(futures), 1):
                result = future.result()
                print(f"EVALUATED {count}/{len(jobs)} {result['route']} {result['lineage']} {result['twin']}", flush=True)
        packets = []
        for route in K.CONFIG["routes"]:
            for lineage in range(K.CONFIG["lineages"]):
                paths = [job_directory(route, lineage, twin) / "endpoints.json.gz" for twin in K.CONFIG["twins"]]
                assert sha(paths[0]) == sha(paths[1]), "endpoint twins diverged"
                packets.extend(load_gzip(paths[0]))
        from training.audit_encephalon_e1 import decide
        verdict = decide(packets)
        verify_manifest()
        path = K.OUT / "raw_verdict.json"
        if path.exists(): assert verdict == read(path)
        else: save(path, verdict)
        print(json.dumps({k: verdict[k] for k in ("e1_verdict", "controller_qualification", "direct_sensing_benefit")}), flush=True)
