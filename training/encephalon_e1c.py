"""E1-C own-action learning with arm-parameterized entropy weight.

Replica of the frozen E1-A trainer except: (a) the arm (entropy01/entropy00)
replaces the route dimension and selects the entropy coefficient; (b) the
actor-critic loss is parameterized by that coefficient instead of hardcoding
.01. Frozen E1-A sources are not imported for training math (only shared
physics/agent/reward helpers); the formula is otherwise identical.
"""
import copy
import hashlib
...
def loss_c(batch, bootstrap, entropy_weight):
    """E1-A actor-critic formula with parameterized entropy coefficient."""
...
class Fit:
    def __init__(self, arm, lineage, config=None, development=False):
        ...
        c = self.config
        if arm not in c["arms"]: raise ValueError(f"unknown E1-C arm {arm}")
        ...
        self.update = 0
        self.history = []
        self.trace = hashlib.sha256(b"E1-C training chain").hexdigest()

    def entropy_weight(self):
        return self.config["entropy_weight"][self.arm]
    ...
    def advance(self, checker=None):
        ...
        batch, state, bootstrap = collect(self.agent, self.worlds, self.state, self.sampler,
                                           c["rollout"], checker)
        total, metrics = loss_c(batch, bootstrap, self.entropy_weight())
...
@torch.no_grad()
def evaluate(model, arm, lineage, profile, control, *, count=None, horizon=None, development=False):
    c = K.CONFIG
