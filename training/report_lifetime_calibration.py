"""Publish the bounded world calibration without revising its frozen sources."""
import json
...
def main():
    ...
    result = read('results.json'); audit = read('audit.json'); completion = read('completion.json')
    assert audit['passed']
    compact = {k: v for k, v in result.items() if k != 'episodes'}
    compact['episodes'] = [{k: v for k, v in row.items() if k not in ('initial', 'final')}
                           for row in result['episodes']]
    for name, data in [('results', compact), ('audit', audit), ('completion', completion)]:
        with (REPORTS / f'lifetime_calibration_v1_{name}_20260909.json').open('x', encoding='utf-8') as f:
            json.dump(data, f, separators=(',', ':'), allow_nan=False)
    table = '| Controller | Stable survival | Changing survival | Mean age, changing |\n| --- | --- | --- | --- |\n'
    for name, values in result['aggregates']['changing'].items():
        stable = result['aggregates']['stable'][name]
        table += f"| {name} | {stable['survivors']}/32 | {values['survivors']}/32 | {values['mean_ticks']:.1f} |\n"
    note = f'''# Lifetime world v1: built; adaptation calibration FAIL
...
The audit checks all {audit['transitions']:,} transitions across all 640 episodes.
...
'''
    (ROOT / 'docs/lifetime_calibration_v1_review_20260909.md').write_text(note, encoding='utf-8')
    print('Wrote calibration review and compact evidence')
