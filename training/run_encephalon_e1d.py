"""Commit-bound E1-D fitting, deterministic repeats and locked endpoint execution."""
import argparse
...
from training.encephalon_e1d import Fit, deterministic, evaluate, read_checkpoint, tree_hash, write_checkpoint
...
def endpoint_job(arm, lineage, twin):
    ...
    for control in K.CONFIG["controls"]:
        for profile in K.CONFIG["profiles"]:
            ...
            packets.append(evaluate(model, arm, lineage, profile, control, "live"))
    if arm == "intent":
        for profile in K.CONFIG["profiles"]:
            if time.time() > manifest["deadline_unix"]: raise TimeoutError("frozen six-hour compute window exhausted")
            for condition in ("clamped", "permuted"):
                packets.append(evaluate(final["model"], arm, lineage, profile, "trained", condition))
    verify_manifest()
...
def run(resume=False):
    ...
    with campaign_lock():
        ...
        from training.audit_encephalon_e1d import decide
        verdict = decide(packets)
        verify_manifest()
        path = K.OUT / "raw_verdict.json"
        if path.exists(): assert verdict == read(path)
        else: save(path, verdict)
        print(json.dumps({k: verdict[k] for k in ("e1_verdict", "selected_arm", "controller_qualification", "intent_attribution")}), flush=True)
