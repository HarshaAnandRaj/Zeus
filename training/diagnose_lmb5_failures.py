...
REPORT=ROOT/'zeus_sandbox/universe/reports/lmb5_failure_diagnosis_20260913.json'
SOURCES=('training/diagnose_lmb5_failures.py','training/test_lmb5_failure_diagnosis.py','docs/lmb5_failure_diagnosis_protocol_20260913.md')
...
def main():
    ...
    steps=sum(g['steps'] for g in groups.values());assert steps==audit['decisions'] and len(summaries)==7168
    ...
    result=dict(status='COMPLETE',verdict='FAIL',pillar_promotion=False,commit=commit,sources={n:M.L.P.L3.sha(ROOT/n) for n in SOURCES},
        ...
        bodies=len(summaries),fatal_bodies=len(fatal),source_steps_replayed=source_steps,steps_replayed=steps,
